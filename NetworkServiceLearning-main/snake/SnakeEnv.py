...
class SnakeEnv(Env):
    ...
    def __init__(self, comm_type='tcp', host='localhost', port=12345):
        ...
        self.observation_space = Dict({
            'snake': Box(0, 100, (MAX_LENGTH, 2), dtype=np.int32),
            'foods': Box(0, 100, (MAX_FOOD_NUMBERS, 2), dtype=np.int32),
            'score': Box(0, 10000, (), dtype=np.int32),
            'walls': Box(0, 1, (MAP_HEIGHT, MAP_WIDTH), dtype=np.int8)
        })
...
if __name__ == "__main__":
    ...
    obs, reward, done, _, _ = env.step('up')
